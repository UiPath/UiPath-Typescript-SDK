...
from holmes.common.env_vars import AZURE_APP_TENANT_ID, AZURE_APP_CLIENT_ID, AZURE_APP_CLIENT_SECRET, KEY_VAULT_URL, KEY_VAULT_SECRET_NAME
...
class ExecuteAzureSQLQuery(Tool):
    ...
    def _invoke(self, params: Dict) -> str:
        try:
            ...
            # Create list of dictionaries for better readability
            results = []
            for row in rows:
                result = {}
...
class AzureSQLQueryToolset(Toolset):
    def __init__(self):
        super().__init__(
            ...
            icon_url="",
            prerequisites=[],  # Simplified to empty list for now
            tools=[
                FetchKeyVaultSecret(),
...
if __name__ == "__main__":
    # First, create instance of Key Vault tool and fetch connection string
    keyvault_tool = FetchKeyVaultSecret()
    
    # Parameters for fetching connection string from Key Vault
    keyvault_params = {
