...
from holmes.common.env_vars import UIPATH_TOKEN, CONFLUENCE_BASE_URL
...
def parse_confluence_content(html_content: str) -> str:
    ...
    for excerpt in soup.find_all('ac:structured-macro', {'ac:name': ['excerpt', 'excerpt-include']}):
        # For excerpt-include, get the referenced content
        if excerpt.get('ac:name') == 'excerpt-include':
            excerpt_body = excerpt.find('ac:rich-text-body')
            if excerpt_body:
                excerpt.replace_with(excerpt_body)
        # For regular excerpts, get the content
        else:
            excerpt_body = excerpt.find('ac:rich-text-body')
            if excerpt_body:
                excerpt.replace_with(excerpt_body)
...
class ConfluenceToolset(Toolset):
    def __init__(self):
        super().__init__(
            ...
            docs_url="",  # Add documentation URL if available
