...
// node_modules/uipath-typescript-sdk/dist/config.js
var ConfigSchema = v3_exports.object({
  baseUrl: v3_exports.string().url().default("https://cloud.uipath.com"),
  orgName: v3_exports.string().min(1),
  tenantName: v3_exports.string().min(1),
  secret: v3_exports.string().optional(),
  clientId: v3_exports.string().optional(),
  redirectUri: v3_exports.string().url().optional(),
  accountId: v3_exports.string().uuid().optional(),
  tenantId: v3_exports.string().uuid().optional(),
  folderKey: v3_exports.string().uuid().optional()
});

// node_modules/uipath-typescript-sdk/dist/utils/constants.js
var ENV = {
  BASE_URL: "UIPATH_URL",
...
// node_modules/uipath-typescript-sdk/dist/executionContext.js
...
// node_modules/uipath-typescript-sdk/dist/services/apiClient.js
var ApiClient = class {
  constructor(config, executionContext, clientConfig = {}) {
    this.defaultHeaders = {};
    this.config = config;
...
// node_modules/uipath-typescript-sdk/dist/services/baseService.js
var BaseService = class {
  ...
  async request(method, path, options = {}) {
    switch (method.toUpperCase()) {
      ...
      case "POST":
        return this.post(path, options.data, options);
      case "PUT":
        return this.put(path, options.data, options);
      case "PATCH":
        return this.patch(path, options.data, options);
      case "DELETE":
  ...
  async get(path, options = {}) {
    const response = await this.apiClient.get(path, options);
    return { data: response };
  }
  async post(path, data, options = {}) {
    const response = await this.apiClient.post(path, data, options);
    return { data: response };
  }
  async put(path, data, options = {}) {
    const response = await this.apiClient.put(path, data, options);
    return { data: response };
  }
  async patch(path, data, options = {}) {
    const response = await this.apiClient.patch(path, data, options);
    return { data: response };
  }
  async delete(path, options = {}) {
    const response = await this.apiClient.delete(path, options);
    return { data: response };
  }
};

// node_modules/uipath-typescript-sdk/dist/folderContext.js
var HEADER_FOLDER_KEY = "X-UIPATH-OrganizationUnitId";
var HEADER_FOLDER_PATH = "X-UIPATH-OrganizationUnitPath";
var FolderContext = class {
  constructor(config, executionContext, folderConfig = {}) {
    this.config = config;
    this.executionContext = executionContext;
   ...
   */
  get folderHeaders() {
    if (this._folderKey !== void 0) {
      return { [HEADER_FOLDER_KEY]: this._folderKey };
    } else if (this._folderPath !== void 0) {
...
// node_modules/uipath-typescript-sdk/dist/services/actionsService.js
var ActionsService = class extends BaseService {
  constructor(config, executionContext) {
   ...
   */
  async create(action) {
    const response = await this.post("/actions", action);
  ...
  async update(id, action) {
    const response = await this.put(`/actions/${id}`, action);
    return response.data;
  }
  /**
   ...
   */
  async retrieve(id) {
    const response = await this.get(`/actions/${id}`);
    return response.data;
  }
  /**
  ...
  async deleteAction(id) {
    await this.delete(`/actions/${id}`);
  }
  /**
   * Lists all actions
   */
  async list() {
    const response = await this.get("/actions");
    return response.data;
  }
  /**
   * Creates a new action from a schema
   */
  async createFromSchema(appName, appFolderPath, actionName, actionSchema) {
    const spec = this.getCreateFromSchemaSpec(appName, appFolderPath, actionName, actionSchema);
   ...
   */
  async retrieveAppSchema(appName, appFolderPath) {
  ...
  getCreateFromSchemaSpec(appName, appFolderPath, actionName, actionSchema) {
  ...
  extractDeployedApp(deployed, appFolderPath) {
    const deployedApp = deployed.find((app) => app.deploymentFolder.fullyQualifiedName === appFolderPath);
    if (!deployedApp) {
      throw new Error(`No deployed app found in folder ${appFolderPath}`);
    }
    return deployedApp;
  }
};

// node_modules/uipath-typescript-sdk/dist/utils/overwritesManager.js
var fs = __toESM(require_fs(), 1);
var OverwritesManager = class _OverwritesManager {
  ...
  static getInstance(overwritesFilePath) {
    if (!_OverwritesManager.instance) {
      _OverwritesManager.instance = new _OverwritesManager(overwritesFilePath);
    } else if (overwritesFilePath && overwritesFilePath !== _OverwritesManager.instance.overwritesFilePath) {
      _OverwritesManager.instance.overwritesFilePath = overwritesFilePath;
      _OverwritesManager.instance.readOverwritesFile();
    }
    return _OverwritesManager.instance;
  }
  readOverwritesFile() {
    try {
...
// node_modules/uipath-typescript-sdk/dist/utils/headers.js
...
// node_modules/uipath-typescript-sdk/dist/services/assetsService.js
var AssetsService = class extends BaseService {
  ...
  async retrieve(name, options = {}) {
    const [overwrittenName, overwrittenFolderPath] = withResourceOverwrites("asset", name, options.folderPath);
  ...
  async update(robotAsset, options = {}) {
    const [overwrittenName, overwrittenFolderPath] = withResourceOverwrites("asset", robotAsset.name || "", options.folderPath);
...
// node_modules/uipath-typescript-sdk/dist/utils/platform.js
var isNode = typeof process !== "undefined" && process.versions != null && process.versions.node != null;
var isBrowser = typeof window !== "undefined" && typeof window.document !== "undefined";
var FileSystem = class {
  static async importNodeFs() {
    if (isNode) {
      const fs2 = await import("./promises-OOZ6ZNCC.js");
      return fs2;
    }
    return null;
  }
  static async readFile(path) {
    if (isNode) {
      const fs2 = await this.importNodeFs();
      if (fs2) {
  ...
  static async writeFile(path, data) {
    if (isNode) {
      const fs2 = await this.importNodeFs();
      if (fs2) {
        return fs2.writeFile(path, data);
      }
    }
    throw new Error("File system operations are only available in Node.js environment");
  }
...
// node_modules/uipath-typescript-sdk/dist/services/bucketsService.js
var BucketsService = class extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
    this.folderContext = new FolderContext(config, executionContext);
  }
   ...
   */
  async download({ name, key, blobFilePath, destinationPath, onDownloadComplete, ...options }) {
    ...
    if (readUri.RequiresAuth) {
      ...
      response = await axios_default.get(readUri.Uri, {
        headers,
        responseType: "arraybuffer"
      });
    }
   ...
   * @param params.folderPath - The path of the folder where the bucket resides
   */
  async upload({ name, key, blobFilePath, contentType, sourcePath, content, ...options }) {
    ...
    if (writeUri.RequiresAuth) {
      ...
      await axios_default.put(writeUri.Uri, formData, { headers });
    }
  }
  /**
  ...
  async uploadFromMemory({ name, key, blobFilePath, contentType, content, ...options }) {
    ...
    if (writeUri.RequiresAuth) {
      ...
      await axios_default.put(writeUri.Uri, data, { headers });
    }
  }
  /**
  ...
  async retrieve({ name, key, ...options }) {
    ...
    const { method, url, params, headers } = key ? this.getRetrieveByKeyConfig(key, options) : this.getRetrieveConfig(name, options);
    try {
      const response = await this.request(method, url, {
        params,
        headers: {
      ...
      if (axios_default.isAxiosError(error)) {
        ...
        throw new Error(`Failed to access bucket ${key || name}. Status: ${(_b = error.response) == null ? void 0 : _b.status}, Message: ${error.message}`);
      }
      throw error;
    }
  }
...
// node_modules/uipath-typescript-sdk/dist/services/entityService.js
var EntityService = class _EntityService extends BaseService {
  /**
  ...
  async getById(id, options = {}) {
    ...
    const url = `${_EntityService.BASE_PATH}/entity/${id}/query_expansion${queryParams.toString() ? `?${queryParams.toString()}` : ""}`;
  ...
  async getByName(name, options = {}) {
    ...
    const url = `${_EntityService.BASE_PATH}/${name}/read${queryParams.toString() ? `?${queryParams.toString()}` : ""}`;
   ...
   * });
   */
  async queryById(id, query, options = {}) {
    const { expansionLevel = 0 } = options;
    const queryParams = new URLSearchParams();
    if (expansionLevel > 0) {
      queryParams.append("expansionLevel", expansionLevel.toString());
   ...
   * });
   */
  async queryByName(name, query, options = {}) {
    const { expansionLevel = 0 } = options;
    const queryParams = new URLSearchParams();
    if (expansionLevel > 0) {
      queryParams.append("expansionLevel", expansionLevel.toString());
    }
    const url = `${_EntityService.BASE_PATH}/${name}/query_expansion${queryParams.toString() ? `?${queryParams.toString()}` : ""}`;
    const response = await this.request("POST", url, {
      data: query,
      headers: {
        "Content-Type": "application/json"
  ...
  async insert(name, data, options = {}) {
    ...
    const url = `${_EntityService.BASE_PATH}/${name}/insert${queryParams.toString() ? `?${queryParams.toString()}` : ""}`;
    const response = await this.request("POST", url, {
      data,
      headers: {
        "Content-Type": "application/json"
...
EntityService.BASE_PATH = "dataservice_/api/EntityService";

// node_modules/uipath-typescript-sdk/dist/services/caseService.js
var CaseDefinitionOperations = class _CaseDefinitionOperations {
  constructor(entityService, definitionName) {
  ...
  async getActiveCases(options = {}) {
    ...
    const query = {
      skip: start,
      top: limit,
      select: selectedFields || _CaseDefinitionOperations.DEFAULT_SELECTED_FIELDS,
      ...
      orderBy: (sortOptions == null ? void 0 : sortOptions.map((sort) => `${sort.fieldName} ${sort.isDescending ? "desc" : "asc"}`)) || ["CreateTime desc"]
    };
    return this.entityService.queryByName("Cases", query, {
      expansionLevel: (expansions == null ? void 0 : expansions.length) ? 1 : 0
    });
  }
  /**
...
CaseDefinitionOperations.DEFAULT_SELECTED_FIELDS = [
  "Id",
  "Name",
  "CaseVersion",
  "SlaHours",
...
var CaseService = class _CaseService extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
   ...
   */
  async list(options = {}) {
    return this.entityService.list(_CaseService.CASE_ENTITY_NAME, options);
  }
};
CaseService.CASE_ENTITY_NAME = "Cases";

// node_modules/uipath-typescript-sdk/dist/services/connectionsService.js
var ConnectionsService = class extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
  }
  /**
...
// node_modules/uipath-typescript-sdk/dist/models/contextGroundingIndex.js
var ContextGroundingDataSourceSchema = v3_exports.object({
  "@odata.type": v3_exports.string(),
  folder: v3_exports.string(),
  bucketName: v3_exports.string(),
  fileNameGlob: v3_exports.string(),
  directoryPath: v3_exports.string()
});
var ContextGroundingIndexSchema = v3_exports.object({
  ...
  status: v3_exports.string().optional(),
  lastIngestionTime: v3_exports.string().optional(),
  ingestionInProgress: v3_exports.boolean().optional()
});

// node_modules/uipath-typescript-sdk/dist/models/contextGrounding.js
var ContextGroundingQueryResponseSchema = v3_exports.object({
  content: v3_exports.string(),
  score: v3_exports.number(),
  metadata: v3_exports.record(v3_exports.unknown())
});

// node_modules/uipath-typescript-sdk/dist/models/errors.js
var IngestionInProgressException = class extends Error {
...
// node_modules/uipath-typescript-sdk/dist/services/contextGroundingService.js
var ContextGroundingService = class extends BaseService {
  ...
  async create(name, storageBucketName, options = {}) {
    ...
    const response = await this.request("POST", ENDPOINTS.CONTEXT_GROUNDING.CREATE, {
      data: {
        name,
   ...
   */
  async addToIndex(name, contentType, blobFilePath, content, options = {}) {
    var _a, _b;
    if (!content) {
    ...
    if (typeof content === "string") {
      await this.bucketsService.uploadFromMemory({
        ...
        contentType,
        folderKey: ((_a = index.dataSource) == null ? void 0 : _a.folder) || ""
      });
    } else {
      await this.bucketsService.upload({
        ...
        folderPath: bucketFolderPath,
        contentType,
        folderKey: ((_b = index.dataSource) == null ? void 0 : _b.folder) || ""
      });
    }
  ...
  async retrieve(name, options = {}) {
    const folderKey = await this.resolveFolderKey(options.folderKey, options.folderPath);
    const response = await this.request("GET", ENDPOINTS.CONTEXT_GROUNDING.INDEXES, {
      params: {
        "$filter": `Name eq '${name}'`,
        "$expand": "dataSource"
      },
    ...
    const indexes = v3_exports.array(ContextGroundingIndexSchema).parse(response.data.value);
  ...
  async search(name, query, numberOfResults = 10, options = {}) {
    ...
    const folderKey = await this.resolveFolderKey(options.folderKey, options.folderPath);
    const response = await this.request("POST", ENDPOINTS.CONTEXT_GROUNDING.SEARCH, {
      data: {
        query: {
          query,
          numberOfResults
    ...
    return v3_exports.array(ContextGroundingQueryResponseSchema).parse(response.data);
  }
  /**
  ...
  async ingestData(index, options = {}) {
    ...
    const folderKey = await this.resolveFolderKey(options.folderKey, options.folderPath);
    try {
      await this.request("POST", `${ENDPOINTS.CONTEXT_GROUNDING.INDEXES}/${index.id}/ingest`, {
        headers: {
          ...headerFolder(folderKey)
        }
      });
    } catch (error) {
      if (error instanceof AxiosError2 && ((_a = error.response) == null ? void 0 : _a.status) === 409) {
  ...
  async deleteIndex(index, options = {}) {
    ...
    const folderKey = await this.resolveFolderKey(options.folderKey, options.folderPath);
    await this.request("DELETE", `${ENDPOINTS.CONTEXT_GROUNDING.INDEXES}/${index.id}`, {
      headers: {
        ...headerFolder(folderKey)
      }
    });
  ...
  async retrieveById(id, options = {}) {
    const folderKey = await this.resolveFolderKey(options.folderKey, options.folderPath);
    const response = await this.request("GET", `${ENDPOINTS.CONTEXT_GROUNDING.INDEXES}/${id}`, {
      headers: {
        ...headerFolder(folderKey)
      }
    });
  ...
  async resolveFolderKey(folderKey, folderPath) {
    ...
    if (!folderKey && !folderPath) {
      folderKey = this.folderContext.folderKey || (this.folderContext.folderPath ? await this.foldersService.retrieveKeyByFolderPath(this.folderContext.folderPath) : void 0);
    }
    if (!folderKey) {
  ...
  extractBucketInfo(index) {
    var _a, _b;
    try {
      if (!((_a = index.dataSource) == null ? void 0 : _a.bucketName) || !((_b = index.dataSource) == null ? void 0 : _b.folder)) {
...
// node_modules/uipath-typescript-sdk/dist/services/debugInstancesService.js
var DebugInstancesService = class _DebugInstancesService extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
   ...
   */
  async createDebugInstances(request) {
    const response = await this.post(_DebugInstancesService.BASE_PATH, request);
    return response.data;
  }
  /**
   * Cancels a debug instance
   */
  async cancelDebugInstance(instanceId, request) {
    const response = await this.post(`${_DebugInstancesService.BASE_PATH}/${instanceId}/cancel`, request);
    return response.data;
  }
  /**
   * Continues a debug instance
   */
  async continueDebugInstance(instanceId, request) {
    await this.post(`${_DebugInstancesService.BASE_PATH}/${instanceId}/continue`, request);
  }
  /**
   * Gets element executions for a debug instance
   */
  async getDebugInstanceElementExecutions(instanceId) {
    const response = await this.get(`${_DebugInstancesService.BASE_PATH}/${instanceId}/element-executions`);
    return response.data;
  }
  /**
   * Gets incidents for a debug instance
   */
  async getIncidentsByInstanceId(instanceId) {
    const response = await this.get(`${_DebugInstancesService.BASE_PATH}/${instanceId}/incidents`);
    return response.data;
  }
  /**
   * Gets variables for a debug instance
   */
  async getDebugVariables(instanceId, parentElementId) {
    const url = `${_DebugInstancesService.BASE_PATH}/${instanceId}/variables`;
    const params = parentElementId ? { parentElementId } : void 0;
   ...
   */
  async updateDebugVariables(instanceId, request, parentElementId) {
    const url = `${_DebugInstancesService.BASE_PATH}/${instanceId}/variables`;
    const params = parentElementId ? { parentElementId } : void 0;
    await this.patch(url, request, { params });
  }
  /**
   * Helper method to find an element in the execution tree
   */
  static findElement(elements, predicate) {
   ...
   */
  async waitForState(instanceId, targetStates, timeoutMs = 3e4, pollIntervalMs = 1e3) {
...
DebugInstancesService.BASE_PATH = "/v1/debug-instances";

// node_modules/uipath-typescript-sdk/dist/services/folderService.js
var FolderService = class extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
  }
  /**
  ...
  async retrieveKeyByFolderPath(folderPath) {
    ...
    return (_a = response.data.PageItems.find((item) => item.FullyQualifiedName === folderPath)) == null ? void 0 : _a.Key;
  }
...
// node_modules/uipath-typescript-sdk/dist/services/jobsService.js
var JobsService = class extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
  }
  /**
...
var UsageInfoSchema = v3_exports.object({
  prompt_tokens: v3_exports.number(),
  completion_tokens: v3_exports.number().optional(),
  total_tokens: v3_exports.number()
});
var TextEmbeddingSchema = v3_exports.object({
  object: v3_exports.string(),
  data: v3_exports.array(v3_exports.object({
    object: v3_exports.string(),
    embedding: v3_exports.array(v3_exports.number()),
    index: v3_exports.number()
  })),
  model: v3_exports.string(),
  usage: UsageInfoSchema
});
var ChatCompletionSchema = v3_exports.object({
  id: v3_exports.string(),
  object: v3_exports.string(),
  created: v3_exports.number(),
  model: v3_exports.string(),
  choices: v3_exports.array(v3_exports.object({
    index: v3_exports.number(),
    message: v3_exports.object({
      role: v3_exports.string(),
      content: v3_exports.string()
    }),
    finish_reason: v3_exports.string()
  })),
  usage: UsageInfoSchema
});
var UiPathOpenAIService = class extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
  }
  /**
...
// node_modules/uipath-typescript-sdk/dist/services/maestroProcessesService.js
var MaestroProcessesService = class extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
  }
  /**
   ...
   */
  async getSummary() {
    const response = await this.get("pims_/api/v1/processes/summary", {});
    return response.data;
  }
  /**
   ...
   */
  async getSettings(processKey) {
    const response = await this.get(`pims_/api/v1/processes/${processKey}/settings`, {});
    return response.data;
  }
};

// node_modules/uipath-typescript-sdk/dist/models/processInstance.js
var GetInstanceResponseSchema = v3_exports.object({
  organizationId: v3_exports.string().nullable(),
  tenantId: v3_exports.string().nullable(),
  instanceId: v3_exports.string().nullable(),
  packageKey: v3_exports.string().nullable(),
  packageId: v3_exports.string().nullable(),
  packageVersion: v3_exports.string().nullable(),
  latestRunId: v3_exports.string().nullable(),
  latestRunStatus: v3_exports.string().nullable(),
  processKey: v3_exports.string().nullable(),
  folderKey: v3_exports.string().nullable(),
  userId: v3_exports.number().int(),
  instanceDisplayName: v3_exports.string().nullable(),
  source: v3_exports.string().nullable(),
  startedByUser: v3_exports.string().nullable(),
  creatorUserKey: v3_exports.string().nullable(),
  startedTimeUtc: v3_exports.string().datetime(),
  completedTimeUtc: v3_exports.string().datetime().nullable(),
  state: v3_exports.string().nullable(),
  priority: v3_exports.string().nullable(),
  content: v3_exports.any(),
  output: v3_exports.any(),
  metadata: v3_exports.any()
});
var InstanceRunSchema = v3_exports.object({
  // TODO: Add InstanceRun schema properties when available
});
var IncidentResponseSchema = v3_exports.object({
  // TODO: Add IncidentResponse schema properties when available
});
var GoToCursorsSchema = v3_exports.object({
  // TODO: Add GoToCursors schema properties when available
});
var SourceEnum = v3_exports.enum(["1", "2"]);
var StatusEnum = v3_exports.enum(["Unset", "Ok", "Error", "Unspecified"]);
var SpanSchema = v3_exports.object({
  id: v3_exports.string().uuid(),
  traceId: v3_exports.string().uuid(),
  parentId: v3_exports.string().uuid().nullable(),
  name: v3_exports.string().nullable(),
  startTime: v3_exports.string().datetime(),
  endTime: v3_exports.string().datetime().nullable(),
  attributes: v3_exports.string().nullable(),
  status: StatusEnum,
  createdAt: v3_exports.string().datetime(),
  updatedAt: v3_exports.string().datetime(),
  organizationId: v3_exports.string().uuid(),
  tenantId: v3_exports.string().uuid().nullable(),
  expiryTimeUtc: v3_exports.string().datetime().nullable(),
  folderKey: v3_exports.string().uuid().nullable(),
  source: SourceEnum
});
var InstanceCancelRequestSchema = v3_exports.object({
  comment: v3_exports.string().nullable()
});
var InstancePauseRequestSchema = v3_exports.object({
  comment: v3_exports.string().nullable()
});
var InstanceResumeRequestSchema = v3_exports.object({
  comment: v3_exports.string().nullable()
});
var InstancesStatusResponseSchema = v3_exports.object({
  instanceId: v3_exports.string().nullable(),
  status: v3_exports.string().nullable()
});
var GetAllInstancesResponseSchema = v3_exports.object({
  instances: v3_exports.array(GetInstanceResponseSchema).nullable(),
  nextPage: v3_exports.string().nullable(),
  hasMoreResults: v3_exports.boolean()
});
var ProcessInstance = class _ProcessInstance {
  constructor(instanceData, service, instanceFolderKey) {
    this.instanceData = instanceData;
    this.service = service;
    this.instanceFolderKey = instanceFolderKey;
  }
...
// node_modules/uipath-typescript-sdk/dist/services/processInstancesService.js
var ProcessInstancesService = class extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
  }
  getFolderKeyHeader(folderKey) {
    return {
   ...
   */
  async getAll(params) {
    const response = await this.get("pims_/api/v1/instances", {
      params
    });
    return response.data;
  }
  ...
  async getById(instanceId, folderKey) {
    const response = await this.get(`pims_/api/v1/instances/${instanceId}`, {
      headers: {
        [HEADERS.FOLDER_KEY]: folderKey
      }
    });
   ...
   */
  async getInstanceById(instanceId, folderKey) {
  ...
  async getExecutionHistory(instanceId) {
    const response = await this.get(`pims_/api/v1/spans/${instanceId}`);
    return response.data;
  }
  /**
  ...
  async getBpmn(instanceId, folderKey) {
    const response = await this.get(`pims_/api/v1/instances/${instanceId}/bpmn`, {
      headers: {
        [HEADERS.FOLDER_KEY]: folderKey,
   ...
   */
  async cancel(instanceId, folderKey, request = { comment: null }) {
   ...
   * @returns Promise<void>
   */
  async pause(instanceId, folderKey, request = { comment: null }) {
   ...
   * @returns Promise<void>
   */
  async resume(instanceId, folderKey, request = { comment: null }) {
...
// node_modules/uipath-typescript-sdk/dist/services/processesService.js
var ProcessesService = class extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
    this.folderContext = new FolderContext(config, executionContext);
  }
...
// node_modules/uipath-typescript-sdk/dist/services/queuesService.js
var QueuesService = class extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
  }
  /**
...
// node_modules/uipath-typescript-sdk/dist/services/insightsService.js
var InsightsService = class extends BaseService {
  constructor(config, executionContext) {
    super(config, executionContext);
  }
  /**
   ...
   */
  async getProcessInstanceStatusByDate(startTime, endTime, timeSliceUnit = "DAY", timezoneOffset = (/* @__PURE__ */ new Date()).getTimezoneOffset(), tenantId) {
    const request = {
      commonParams: {
        startTime,
        endTime
    ...
    const response = await this.post("insightsrtm_/agenticInstanceStatus/uipathlabs/Playground/bd829329-42ff-40aa-96dc-95a78168275a/InstanceStatusByDate", request);
    return response.data;
  }
};

// node_modules/uipath-typescript-sdk/dist/services/authService.js
...
var AuthService = class extends BaseService {
  ...
  generateCodeVerifier() {
    if (isBrowser2) {
      ...
      return crypto2.randomBytes(32).toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
    }
  }
  /**
  ...
  async generateCodeChallenge(codeVerifier) {
    if (isBrowser2) {
      ...
      const crypto2 = require_crypto();
      return crypto2.createHash("sha256").update(codeVerifier).digest("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
    }
  }
  /**
...
// node_modules/uipath-typescript-sdk/dist/uipath.js
...
var UiPath = class {
  constructor(config) {
    ...
    try {
      ...
      if (error instanceof v3_exports.ZodError) {
...
// node_modules/uipath-typescript-sdk/dist/models/insights.js
var TimeSliceUnitEnum = v3_exports.enum(["DAY", "HOUR", "MINUTE"]);
var CommonParamsSchema = v3_exports.object({
  startTime: v3_exports.number(),
  endTime: v3_exports.number()
});
var ProcessInstanceStatusByDateRequestSchema = v3_exports.object({
  commonParams: CommonParamsSchema,
  timeSliceUnit: TimeSliceUnitEnum,
  timezoneOffset: v3_exports.number(),
  tenantId: v3_exports.string().uuid()
});
var ProcessInstanceStatusByDateItemSchema = v3_exports.object({
  startTime: v3_exports.string(),
  status: v3_exports.string(),
  count: v3_exports.number()
});
var ProcessInstanceStatusByDateResponseSchema = v3_exports.array(ProcessInstanceStatusByDateItemSchema);

// node_modules/uipath-typescript-sdk/dist/sync/SyncManager.js
...
var SyncManager = class {
  constructor(options) {
    ...
    this.entitySubscriptions = /* @__PURE__ */ new Set();
    this.options = {
  ...
  async sendUpdate(entity, update) {
    if (!this.ws || this.ws.readyState !== import_ws.default.OPEN) {
...
// node_modules/uipath-typescript-sdk/dist/sync/EntitySyncHandler.js
var EntitySyncHandler = class {
  constructor(syncManager, entityName) {
  ...
  subscribe(onUpdate) {
    ...
    this.unsubscribe = this.syncManager.subscribe(this.entityName, (update) => {
  ...
  handleUpdate(update) {
    ...
    switch (type) {
      case "CREATE":
        ...
        break;
      case "UPDATE": {
        const index = this.state.items.findIndex((item) => this.getEntityId(item) === this.getEntityId(data));
        if (index !== -1) {
      ...
      case "DELETE": {
        this.state.items = this.state.items.filter((item) => this.getEntityId(item) !== this.getEntityId(data));
        break;
      }
    }
  ...
  async create(data) {
    await this.syncManager.sendUpdate(this.entityName, {
      type: "CREATE",
...
//# sourceMappingURL=uipath-typescript-sdk.js.map
